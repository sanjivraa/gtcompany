...
const services = [
  { icon: Brain,      title: "AI Development",        color: "#FF6B00", bg: "rgba(255,107,0,0.08)",  border: "rgba(255,107,0,0.18)",  tags: ["Machine Learning","NLP","Computer Vision","AutoML"],    serviceKey: "ai",
    desc: "Custom AI/ML models, NLP systems, computer vision, and intelligent automation that transform your business operations." },
  { icon: Globe,      title: "Web Development",        color: "#1A8FFF", bg: "rgba(26,143,255,0.07)",  border: "rgba(26,143,255,0.18)",  tags: ["React","Next.js","Node.js","TypeScript"],              serviceKey: "web",
    desc: "High-performance web applications built with cutting-edge frameworks, optimized for speed, SEO, and exceptional UX." },
  { icon: Smartphone, title: "Mobile App Development", color: "#FF3B30", bg: "rgba(255,59,48,0.08)",  border: "rgba(255,59,48,0.18)",  tags: ["Flutter","React Native","Kotlin","Swift"],              serviceKey: "mobile",
    desc: "Native and cross-platform mobile applications for iOS and Android with stunning UI and seamless performance." },
  { icon: Cloud,      title: "Cloud Solutions",        color: "#0A6FD4", bg: "rgba(10,111,212,0.08)", border: "rgba(10,111,212,0.18)", tags: ["AWS","Azure","GCP","DevOps"],                           serviceKey: "cloud",
    desc: "Scalable cloud architecture, migration strategies, and managed services on AWS, Azure, and Google Cloud." },
  { icon: Server,     title: "Enterprise Software",    color: "#10B981", bg: "rgba(16,185,129,0.08)", border: "rgba(16,185,129,0.18)", tags: ["ERP","CRM","BI","Microservices"],                       serviceKey: "enterprise",
    desc: "Custom ERP, CRM, and business intelligence platforms engineered for enterprise-scale operations and growth." },
  { icon: Palette,    title: "UI/UX Design",           color: "#F59E0B", bg: "rgba(245,158,11,0.08)", border: "rgba(245,158,11,0.18)", tags: ["Figma","Design Systems","Prototyping","Research"],      serviceKey: "design",
    desc: "Premium design systems, user research, and pixel-perfect interfaces that convert visitors into loyal customers." },
];

const scrollToContact = (serviceKey) => {
  const el = document.querySelector('#contact');
  if (el) {
    el.scrollIntoView({ behavior: 'smooth' });
    // Pre-select the service in the contact form via a custom event
    setTimeout(() => {
          ...
          {services.map((s, i) => (
            <StaggerChild key={i} dir="up">
              <motion.div whileHover={{ y: -8, borderColor: s.border.replace("0.18", "0.4") }}
                style={{ background: s.bg, border: `1px solid ${s.border}`, borderRadius: "20px", padding: "28px", cursor: "pointer", position: "relative", overflow: "hidden", transition: "all 0.3s", backdropFilter: "blur(8px)", height: "100%", display: "flex", flexDirection: "column" }}
                onClick={() => scrollToContact(s.serviceKey)}>
                ...
                <p style={{ fontSize: "13px", color: "rgba(255,255,255,0.55)", lineHeight: "1.7", marginBottom: "18px", flex: 1 }}>{s.desc}</p>
